...
class Seraph {
  ...
  _processResult(result, singleOrNull) {
    console.log('my brother', result, 'this is goal');
  ...
  _run(query, params) {
    ...
    _readRun(query, params) {
    var sess = this._session();
    return this._treatParams(params)
      .then(params => sess.readTransaction(tx => tx.run(query)))
      .then(this._endSession(sess))
      .catch((e) => {
        this._endSession(sess)();
        return new Promise((_, r) => r(e));
    ...
    _writeRun(query, params) {
    var sess = this._session();
    return this._treatParams(params)
      .then(params => sess.run(query))
      .then(this._endSession(sess))
      .catch((e) => {
        this._endSession(sess)();
        return new Promise((_, r) => r(e));
  ...
  batch() {
   ...
   query(cypher, params) {
    return this._readRun(cypher, params)
      .then((result) => this._processResult(result));
  }

  readQuery(cypher, params) {
    return this._readRun(cypher, params)
      .then((result) => this._processResult(result));
  }

  writeQuery(cypher, params) {
     return this._writeRun(cypher, params)
      .then((result) => this._processResult(result));
  }
